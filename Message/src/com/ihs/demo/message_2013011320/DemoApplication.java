...
public class DemoApplication extends HSApplication implements HSMessageChangeListener, INotificationObserver {
     ...
     */
    void messageFilter(List<HSBaseMessage> messages){
    	String myMid = HSAccountManager.getInstance().getMainAccount().getMID();
    	List<HSBaseMessage> deleteMessages = new ArrayList<HSBaseMessage>();
    	for (int i = 0;i < messages.size();i++)
    		switch (MyDBManager.getAlertSetting(messages.get(i).getFrom())){
    		case 1:
    	...
    	HSMessageManager.getInstance().deleteMessages(deleteMessages);
    ...
    public void onMessageChanged(HSMessageChangeType changeType, List<HSBaseMessage> messages) {
    	...
    	if (changeType == HSMessageChangeType.ADDED && !messages.isEmpty()) {
        	//MainActivity.recentMainActivity.pullNotification();
    		messageFilter(messages);
        	if (HSSessionMgr.getTopActivity() == null) {// 在这里做的通知
        		for (HSBaseMessage message:messages)
        			MainActivity.recentMainActivity.buildNotification(message);            	
            }
        	else{
        	...
        	if (MessagesFragment.adapter != null){
        		for (int i = 0; i < messages.size();i++)
        			MessagesFragment.adapter.update(messages.get(i));
        		MessagesFragment.adapter.notifyDataSetChanged();
        	}
